...
test("liveness stays healthy while readiness reflects database availability", async () => {
  ...
  assert.equal(healthResponseSchema.parse(unavailable.body).status, "not_ready");
  ready = true;
...
test("invalid JSON is a client error and does not expose parser details", async () => {
  const response = await request(createApp({ isReady: () => true }))
    .post("/api/v1/unknown").set("Content-Type", "application/json").send('{"secret":').expect(400);
...
test("unexpected dependency errors are sanitized", async () => {
  const response = await request(createApp({ isReady: () => { throw new Error("private-connection-string"); } }))
    .get("/api/v1/health/ready").expect(500);
  assert.equal(apiErrorSchema.parse(response.body).error.code, "INTERNAL_ERROR");
...
test("environment validation rejects invalid values without exposing credentials", () => {
  assert.throws(() => readEnv({ PORT: "0", MONGODB_URI: "private-credentials" }), /PORT, MONGODB_URI/);
  assert.equal(readEnv({ MONGODB_URI: "mongodb://127.0.0.1:27017/test" }).PORT, 4000);
});
