...
فروشگاه چندتأمین‌کننده‌ای با مدیریت مرکزی. ساختار پروژه و بک‌اند ورود با موبایل پیاده شده‌اند؛ فرم ورود و صفحهٔ حساب نیز پیاده شده و با آزمون مرورگر بررسی شده‌اند. ارسال پیامک واقعی متصل نیست. فروش، پرداخت، قیمت‌گذاری، کیف پول و تسویه مراحل بعدی هستند.
...
صفحهٔ `/login` شامل شمارهٔ موبایل، کد شش‌رقمی، ارسال مجدد و ویرایش شماره است؛ `/account` حساب جاری و خروج را نمایش می‌دهد. `API_ORIGIN` فقط در سرور Next مصرف می‌شود و rewrite مسیر `/api/v1` را به Express می‌فرستد. نشانی مورد استفادهٔ مرورگر باید با `WEB_ORIGIN` بک‌اند دقیقاً یکسان باشد. برای نمونهٔ پیش‌فرض از `http://localhost:3000` استفاده کنید، نه `127.0.0.1`. جزئیات آزمون مرورگر در [راهنمای فرانت](docs/auth-ui.md) آمده است.
